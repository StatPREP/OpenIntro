...
```{r reloading data} 
# DTK: I'm not sure what this is intended to do. Do you want them to copy it into the code chunk?
# Option, make this a -setup chunk for restrict-to-num-char
# Easier, put it in the setup chunk at the top of the document. 
# That's what I did, which means you don't need this chunk at all.
Email_50 <- read.csv("https://nicolelang.github.io/Math1130/email50.csv")
```
...
## Narrowing it down (DTK Style)


NOTE to NICOLE: What I'm doing here is to use R functions that handle variable names concisely, without the need to use dollar signs or quotation marks. 

Many data sets come with more information than we can comfortably view on one screen. For now, we want to work with only one or two variables at a time.

This is how we tell R that we want just the `num_char` variable from the `Email_50` dataframe. (Press "Run" to execute the command.) `num_char`  tells us the number of characters in each email message. 
Note: the values of this variable represent *thousands* of characters. So a value of 2.1 really means there were 2,100 characters in the email.


```{r restrict-to-num-char_dtk, exercise=TRUE}
select(Email_50, num_char)
```

Similarly, you can select two variables. For instance, here is the command to pull out the data used on p. 27 of your textbook. (Again, press "Run" to execute the command.)

```{r new-table_dtk, exercise=TRUE}
select(Email_50, num_char, line_breaks)
```

Now your turn .... In the next box, give the command to pull out `num_char`, `line_breaks`, and `dollar` from the Email_50 dataframe. You can use "Run" to try out any command you compose. If you press "Submit," the tutorial will figure out if you've done the right thing and give you a hint if you haven't.

```{r new-table_dtk_2, exercise = TRUE}

```

```{r new-table_dtk_2-check}
...
dat <- arg_number(res, 1, insist(E == Email_50, "The first argument to `select()` should be `Email_50`, the name of the dataframe."))
...
## Looking ahead: graphical displays of data (DTK style)
...
```{r column-names_dtk, exercise = TRUE}
names(Email_50)
```

Read through the names and try to figure out the name of the variable counting exclamation marks in the message. (There's also a variable counting exclaimation marks in the email subject. That's not what we want.)
